...
class FontTexture : IDisposable {
	...
	public FontTexture(string name, Bitmap image, bool generateMipmaps, bool srgb) {
		Name = name;
		Width = image.Width;
		Height = image.Height;
	...
	public FontTexture(string name, int GLTex, int width, int height, int mipmaplevels, SizedInternalFormat internalFormat) {
		Name = name;
		GLTexture = GLTex;
		Width = width;
		Height = height;
	...
	public FontTexture(string name, int width, int height, IntPtr data, bool generateMipmaps = false, bool srgb = false) {
		Name = name;
		Width = width;
		Height = height;
		InternalFormat = srgb ? Srgb8Alpha8 : SizedInternalFormat.Rgba8;
	...
	public void SetAnisotropy(float level) {
		...
		GL.TextureParameter(GLTexture, TEXTURE_MAX_ANISOTROPY, level.Clamp(1, MaxAniso));
	}
